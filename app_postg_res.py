...
def get_rekap_from_spreadsheet():
    ...
    try:
        ...
        url_sheet = "https://docs.google.com/spreadsheets/d/1jfruOftY0v5uBwx2NctrhczqYWLxmmJzhTU6pwlDRTQ/edit?usp=sharing"
        sheet = client.open_by_url(url_sheet).worksheet("rekapitulasi_data_kabkota")
        data = sheet.get_all_records()
        CACHE_REKAP["data"] = data
...
def get_stok_from_spreadsheet():
    # Cek cache (misal refresh tiap 5 menit/300 detik)
    if time.time() - CACHE_STOK["timestamp"] < 300 and CACHE_STOK["data"]:
        return CACHE_STOK["data"]
    ...
    try:
        ...
        client = gspread.authorize(creds)
        url_sheet = "https://docs.google.com/spreadsheets/d/1jfruOftY0v5uBwx2NctrhczqYWLxmmJzhTU6pwlDRTQ/edit?usp=sharing"
        sheet = client.open_by_url(url_sheet).worksheet("stok_gudang")
        data = sheet.get_all_records()
        CACHE_STOK["data"] = data
        CACHE_STOK["timestamp"] = time.time()
        return data
    except Exception as e:
        print(f"[GSPREAD] Error mengambil data: {e}")
        return []
...
def map_view():
    ...
    stok_gudang = []
    try:
        stok_gudang = get_stok_from_spreadsheet()
    except Exception as e:
